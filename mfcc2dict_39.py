...
def makelabelarray(dim,index):
    labelarray = np.zeros([1,dim])
    labelarray[0,index] = 1
    return labelarray

def make_labels_dict(phones_39, dict48_39,dim=39):
    g = open('train.lab','r')
    dicty_data = {}
    wavname = ''
    for lines in g.readlines():
        ...
        if wavname != lines[0:find_all(lines,'_')[1]]:
            ...
            dicty_data[wavname] = np.empty([0,dim])
...
if __name__ == '__main__':
    ...
    phones_39, dict48_39 = phoneslist()
    dicty_data = make_labels_dict(phones_39, dict48_39)
    np.save('best_x_data.npy',dictx_data)
    np.save('best_y_data.npy',dicty_data)
